...
export async function GET(
  ...
  try {
    ...
    const videoCourse = await db.collection('video-courses').findOne({ _id: new ObjectId(params.id) });
  ...
  try {
    ...
    if (body.lessons) {
      updateData.lessons = body.lessons.map((lesson, index) => ({
        ...lesson,
        id: lesson.id || 'xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx'.replace(/[xy]/g, function(c) {
...
export async function DELETE(
  ...
  try {
    ...
    const result = await db.collection('video-courses').deleteOne({ _id: new ObjectId(params.id) });
